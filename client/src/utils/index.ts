...
export async function parseGQLEntry(entry: any): Promise<any> {
	...
	const fullParsedQueries = await Promise.all(
		parsedQueries.map(async (parsedQuery, i) => {
			...
			if (body) {
				const { data: resData, errors: resErrors } = body
				if (resErrors?.length) {
					reply.response.isError = true
				}
...
function getEntryInfo(entry: HAREntry): BaseEntry {
	...
	const statusMessage = (httpStatus as any)[+status]
...
export function formatData(data: any, mimeType: string) {
