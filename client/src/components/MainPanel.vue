		...
		<DialogRoot v-model:open="serverAddressInputOpened"">
			...
			<DialogPortal>
				...
				>
					...
					<DialogDescription class="leading-normal text-gray10">
						Input your server address to inspect here.
					</DialogDescription>
					<div class="flex flex-col gap-4">
						...
						<div class="flex items-center justify-between space-x-2">
							<Button class="w-full" :class="settings.doCollect ? 'cursor-not-allowed': ''" size="normal" @click="confirmCollectAddress">Get Messages</Button>
							<Button class="w-full" size="normal" @click="confirmServerAddress">{{ settings.isServerConnected ? 'Connected' : 'Subscribe' }}</Button>
						</div>
					</div>
				</DialogContent>
			</DialogPortal>
			...
			<DialogPortal>
				...
				>
					...
					<div class="flex flex-col gap-4">
						...
						<div class="flex items-center justify-between space-x-2">
							<label class="flex flex-col text-left text-sm font-medium text-gray-11 space-y-0.5">Requests
								sorting</label>
							<Select
