...
<template>
	<div class="query-block h-full flex flex-col of-hidden bg-gray1">
		...
		>
			<template #left>
				<div v-if="entry" class="hide-scrollbar ml-2 of-auto whitespace-nowrap text-gray10 text-xs">
					{{ entry.request.url }}
				</div>
			</template>
			<template #right>
				<div
					v-if="isGQLEntry(entry) && entry.request.batch"
					class="rounded-sm px-1.5 py-1 text-tiny font-bold uppercase text-amber8"
				>
					<span class="text-amber8">{{ entry.request.batch.count }}/{{ entry.request.batch.length }}</span>
					in Batch
				</div>
			</template>
		</TopBar>
		<div v-if="entry" class="relative flex flex-1 flex-col of-hidden">
			<template v-if="activeView === 'query'">
				<Code ref="codeRef" :code="parsedQuery as string" :mode="mode" class="of-auto" />
			</template>
			<Table v-else-if="activeView === 'headers'" :items="entry.request.headers" class="px-2 py-1" />
		</div>
	</div>
</template>
